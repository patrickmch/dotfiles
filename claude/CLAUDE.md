...
- **Unified messaging MCP (gc, all platforms)**: the `matrix` MCP server at `http://127.0.0.1:18080/` (launchd `com.mchey.matrix-mcp`, registered at Claude user scope on gc) exposes the whole Matrix/Synapse bridge stack — **Discord (incl. MTRO server), iMessage, WhatsApp, Slack (Anagram), Google Voice** — as one tool surface. Key tools: `matrix.v1.conversations.list` (cross-platform unified inbox, newest first), `matrix.v1.messages.search` (full-text across all platforms), `matrix.v1.timeline.messages.list` (read a room), `matrix.v1.messages.send_text` / `reply_text` (REAL outbound messages as Patrick — Hard Rules apply: draft, show Patrick, explicit approval, then send; never skip). Prefer this MCP for cross-platform message reading/triage. The guarded CLI below adds allowlist+audit mechanics for sends when wanted. Full doc: `~/projects/infrastructure/wiki/services/matrix-mcp.md`.
