#!/bin/bash
# Poll local agent status, pending approvals, and CrowdSolve state.
# Runs as a SessionStart hook on turtle — all checks are local (no SSH needed).
# On air, the original version SSHes to turtle. This version runs directly.

# Detect if we're on turtle (local) or air (remote)
if [[ "$USER" == "tmac" || "$(hostname -s)" == "turtle-1" ]]; then
  # Running on turtle — all local
  LOCAL=true
else
  # Running on air — SSH to turtle
  LOCAL=false
